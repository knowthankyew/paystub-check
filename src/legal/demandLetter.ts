...
- ${stateRule.stateName} Statutory Authorities (${stateRule.statuteRef})`;
  ...
  if (analysis.flsaCompliance.hasIllegalShortageDeduction) {
    statutoryReferences += `\n- 29 C.F.R. § 531.35 Unlawful Deduction of Cash Register Shortages / Uniform Costs.`;
  }
  if (!analysis.flsaCompliance.isExemptThresholdMet) {
    statutoryReferences += `\n- 29 U.S.C. § 213(a)(1) FLSA Salary Exemption Threshold Violations.`;
  }
  if (stateRule.waitingTimePenaltyDays) {
    statutoryReferences += `\n- ${stateRule.stateName} Waiting Time Penalty Provisions (Up to 30 Days Wages).`;
  }
