 ...
 * 2. Raw paystub texts, salary records, or employee PII payloads are strictly forbidden from attributes.
...
export function getPrivacyClaims(report: PrivacyAuditReport): PrivacyClaims {
  if (report.isLocalOnlyHonest) {
    return {
      ...
      disclaimerExecutionText: 'All wage calculations execute 100% locally in your browser with zero remote network transmission.',
      footerTitle: '100% Local Air-Gapped Wage Statement & FLSA Audit Engine.',
      footerSubtext: 'Zero Telemetry • Zero Remote Wage Data Egress • FLSA & State Wage Reality Engine',
  ...
  return {
    ...
    footerSubtext: 'Enterprise Telemetry Mode • Operational Metadata Export Active • Paystubs Air-Gapped',
      ...
      await fetch(this.config.otlpEndpoint, {
        ...
        body: JSON.stringify({
          resourceSpans: [
            {
              resource: {
                attributes: [{ key: 'service.name', value: { stringValue: 'paystub-check' } }],
              },
              scopeSpans: [
                {
                  spans: [
